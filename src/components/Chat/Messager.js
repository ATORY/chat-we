...
function Messager({ socket, phone, chat}) {
    ...
    function sendMessage() {
      // console.log(socket)
      socket.emit('chat', { from: phone, to: chat.id, data: { msg } })
      setMsg('')
    }
  ...
  export default function Index({ chatWith }) {
    return (
      <ChatContext.Consumer>
        {({ socket }) => {
          ...
          return (
            <Messager socket={socket} phone={'phone'} chat={chatWith}/>
          );
        }}
      </ChatContext.Consumer>
    );
